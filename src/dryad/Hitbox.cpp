...
std::pair<std::vector, Collision> Hitbox::getMaximumClearDistanceForVectorFromGameObject(std::vector<std::shared_ptr<Hitbox>> objects, Vector vector)
{
	bool clear = true;
	Vector minimumLineGlobal(Point(650000, 650000));
	Point collPoint;
	for (std::shared_ptr<Hitbox> o : objects) {
		Vector vecCopy = Vector(Point(vector.x, vector.y));
		Point myCenter = getCenter() + *parentPosition;
		double x = myCenter.x;
		double y = myCenter.y;
		std::shared_ptr<Hitbox> targetHitbox = o;
		...
		if (usesCircleHitbox and targetHitbox->usesCircleHitbox) {
			...
			std::shared_ptr<Point> collision = std::make_shared<Point>();
	...
	if (clear) {
		...
		return std::make_pair(vector, c);
	}
	else if (minimumLineGlobal.x < 640000 && minimumLineGlobal.y < 640000) {
		Collision c = { true, collPoint };
		return std::make_pair(minimumLineGlobal, c);
	}
	Collision c = { true, collPoint };
	return std::make_pair(Vector(Point(0, 0)), c);
}

//relative to parent position
std::vector<Point> Hitbox::getRectangularPointsSet() {
...
//relative to parent position
std::vector<Point> Hitbox::getCorners() {
	std::vector<Point> points;
	if (!usesCircleHitbox) {
		Point cCorner = rotate(Point(hitboxRect.x, hitboxRect.y), rotation);
...
std::vector<Point> Hitbox::getEquallySpacedPointsAlongLine(Point origin, Vector a, int pts) {
	std::vector<Point> points;
...
bool Hitbox::testCircleCollision(std::shared_ptr<Hitbox> h1, std::shared_ptr<Hitbox> h2)
{
...
//circle first
bool Hitbox::testHybridCollision(std::shared_ptr<Hitbox> h1, std::shared_ptr<Hitbox> h2)
{
...
bool Hitbox::testFixedRectangleCollision(std::shared_ptr<Hitbox> h1, std::shared_ptr<Hitbox> h2)
{
...
bool Hitbox::lineRect(double x1, double y1, double x2, double y2, double rx, double ry, double rx2, double ry2, std::shared_ptr<Point> nearestCollision) {

	// check if the line has hit any of the rectangle's sides
	// uses the Line/Line function below
	std::shared_ptr<Point> collision = std::make_shared<Point>();
	bool left = lineLine(x1, y1, x2, y2, rx, ry, rx, ry2, collision);
	std::shared_ptr<Point> collision2 = std::make_shared<Point>();
	bool right = lineLine(x1, y1, x2, y2, rx2, ry, rx2, ry2, collision2);
	std::shared_ptr<Point> collision3 = std::make_shared<Point>();
	bool top = lineLine(x1, y1, x2, y2, rx, ry, rx2, ry, collision3);
	std::shared_ptr<Point> collision4 = std::make_shared<Point>();
	bool bottom = lineLine(x1, y1, x2, y2, rx, ry2, rx2, ry2, collision4);
	std::shared_ptr<Point> points [4] = { collision, collision2, collision3, collision4 };
...
bool Hitbox::lineLine(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, std::shared_ptr<Point> collision) {
...
// LINE/CIRCLE
bool Hitbox::lineCircle(double x1, double y1, double x2, double y2, double cx, double cy, double r, std::shared_ptr<Point> nearestCollision) {
	/*
