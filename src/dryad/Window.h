...
public:
	...
	Window(std::string, int width, int height, Resolution res);
	Window(std::string title, Resolution resolution) : Window(title, 0, 0, resolution) {}
	...
	static std::pair<int, int> calculateResolution(Resolution res);
	static std::pair<int, int> calculateResolution(Resolution* res);
private:
