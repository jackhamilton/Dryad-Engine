...
GameLoop::GameLoop(int fps, std::shared_ptr<World> world, std::shared_ptr<Input> input)
{
	...
	world->lastFrameTimeMS = std::make_shared<long double>(lastFrameTimeMS);
	world->defaultFps = std::make_shared<long double>(GameLoop::fps);
}
...
void GameLoop::setInput(std::shared_ptr<Input> input) {
	GameLoop::input = input;
}

void GameLoop::setWorld(std::shared_ptr<World> world)
{
	GameLoop::world = world;
}
