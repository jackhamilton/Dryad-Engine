...
{
public:
	Button(function<void()> onClick, std::shared_ptr<Sprite> sprite, Point position, Size size);
	Button(function<void()> onClick, std::string text, std::string font, int fontSize, SDL_Color textColor, std::shared_ptr<AssetLibrary> assetLibrary, SDL_Color backgroundColor, Point position, Size size);
	//Initializes at text size plus padding
	Button(function<void()> onClick, std::string text, std::string font, int fontSize, SDL_Color textColor, std::shared_ptr<AssetLibrary> assetLibrary, SDL_Color backgroundColor, Point position) : Button(onClick, text, font, fontSize, textColor, assetLibrary, backgroundColor, position, { 0, 0 }) {};
