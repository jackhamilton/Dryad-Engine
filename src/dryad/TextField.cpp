...
TextField::TextField(std::string text, std::string font, int fontSize, Rectangle position, std::shared_ptr<AssetLibrary> assetLibrary)
{
	TextField::font = font;
	...
	isUIElement = true;
	isTextField = true;
	cSpritePointer = std::shared_ptr<CompositeSprite>(new CompositeSprite(1));
	createTextures(cSpritePointer);
	setSprite(cSpritePointer);
	hasMouseClickEvent = true;
	size = { position.width, position.height };
	
	mouseClickEvent = std::bind(&TextField::activate, this);
	onExitingScene = std::bind(&TextField::deactivate, this);
}
...
void TextField::insert(std::string s)
{
...
void TextField::createTextures(std::shared_ptr<CompositeSprite> s) {
	...
	if (cursorX < placement.width - xPlacementOffset && selected) {
		std::pair<SDL_Surface*, Point> cursor = Line::drawLine(Point(cursorX, min(5, placement.height)), Point(cursorX, max(placement.height - 5, 0)), { 0, 0, 0 });
