...
Input::Input(std::shared_ptr<Mouse> mouse)
{
	Input::mouse = mouse;
	isLButtonDown = false;
	isRButtonDown = false;
...
//Adds a collection of pairs of keycodes and eventtypes matched with a std::function to call on that combo.
...
void Input::handleInput(GameLoop* gameLoop)
{
	...
	{	
		...
		case SDL_MOUSEMOTION:
		{
			...
			for (std::function<void(Point)> func : mouse->mouseMovementEvents) {
				func(activePoint);
			}
			if (mouse->activeScene && mouse->sceneObjectSet) {
				auto objects = mouse->sceneObjectSet;
				auto it = objects->begin();
				std::vector<Rectangle> invalidRects;
				for (; it != objects->end(); it++) {
					auto it2 = it->second.begin();
					for (; it2 != it->second.end(); it2++) {
						std::vector<std::shared_ptr<GameObject>> objectAndChildren;
						objectAndChildren.push_back(*it2);
						for (std::shared_ptr<GameObject> g : (*it2)->getChildrenFlat()) {
							objectAndChildren.push_back(g);
						}
						for (std::shared_ptr<GameObject> o : objectAndChildren) {
		...
		case SDL_MOUSEBUTTONDOWN:
		{
			if (event.button.button == SDL_BUTTON_LEFT) {
				...
				SDL_GetMouseState(&x, &y);
				Point activePoint = Point(x, y);
				for (std::function<void(Point)> func : mouse->mouseClickEvents) {
					func(Point(x, y));
				}
				...
				SDL_GetMouseState(&x, &y);
				Point activePoint = Point(x, y);
				for (std::function<void(Point)> func : mouse->mouseRightClickEvents) {
					func(Point(x, y));
				}
				if (mouse->activeScene && mouse->sceneObjectSet) {
					auto objects = mouse->sceneObjectSet;
		...
		case SDL_MOUSEBUTTONUP:
		{
			if (event.button.button == SDL_BUTTON_LEFT) {
				...
				SDL_GetMouseState(&x, &y);
				Point activePoint = Point(x, y);
				for (std::function<void(Point, Point)> func : mouse->mouseClickUpEvents) {
				...
				SDL_GetMouseState(&x, &y);
				Point activePoint = Point(x, y);
				for (std::function<void(Point, Point)> func : mouse->mouseRightClickUpEvents) {
...
std::shared_ptr<Mouse> Input::getMouse()
{
	return mouse;
}
