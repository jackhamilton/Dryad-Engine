...
{
    std::pair<SDL_Surface*, Point> surface = drawLine(positionA, positionB, color);
    position = surface.second;
	std::shared_ptr<Sprite> s = std::shared_ptr<Sprite>(new Sprite({ surface.first }));
	setSprite(s);
    isUIElement = true;
}

std::pair<SDL_Surface*, Point> Line::drawLine(Point positionA, Point positionB, SDL_Color color) {
    Point a, b;
