...
void Renderer::computeLighting(double globalIllumination, std::pair<std::vector<Light>, std::vector<std::vector<Polygon>>> masks)
{
    SDL_Surface* surface;
    std::pair<int, int> dim = Window::calculateResolution(res);
    ...
    std::vector<Light> lights = masks.first;
    std::vector<std::vector<Polygon>> lightMasks = masks.second;
    for (int x = 0; x < lights.size(); x++) {
        Light light = lights.at(x);
        std::vector<Polygon> maskPolys = lightMasks.at(x);
...
bool Renderer::DrawFilledPolygon(Polygon poly, const SDL_Color color) {
    ...
    std::vector<std::pair<Point, Point>> sides = poly.getSides();
    for (std::pair<Point, Point> sideLine : sides) {
